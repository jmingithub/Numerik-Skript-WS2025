...
\includegraphics[scale=0.3]{lib/Richtungsfeld Räuber-Beute-Modell.png} \\
...
\\
\textbf{Hier fehlt ein Donnerstag}

\section{Orbits und Flüsse - nichtlineare autonome Differentialgleichungen}
\subsection{}
\subsection{}
\subsection{}
\subsection{}
\subsection{Typen von \LSG bzw. Orbits}
Gelte 12.1. Für jedes $\psi \in \Omega$ hat $X_\psi$ genau eine der folgenden Eigenschaften: 
......

\subsection{Definition: Fluss}
Ein Fluss ist eine Familie von Abbildungen $F_t:\Omega \to \Omega$ mit $F_0 = {I_d}_\Omega$ und $F_{t_2+t_1} = F_{t_2} \dot F_{t_1} \forall t_1,t_1 \in \mathbb{R}$. \\
Bemerkung:
\begin{enumerate}
\item Ein Fluss ist eine Gruppe. Insbesondere gilt $(F_t)^{-1} = F_{-t} \forall t \in \mathbb{R}$, d.h. jede Abbildung $F_t$ ist bijektiv.
\item Man schreibt oft auch $F(t,x)$ statt $F_t(x)$ und konkret betrachtet man dann einen Fluss als Abbildung $F:\mathbb{R} \times \Omega \to \Omega$
\end{enumerate}

\subsection{Theorem: Glatter Fluss einer Differentialgleichung}
(Ich benutze hier $\psi$ statt dem Zeichen, das sie in der VL verwendet hat.) \\
Die \DGL in $(\star) x'(t) = f(x(t))$ erzeugt einen Fluss via $F_t(\psi) := x_\psi(t) \forall t \in \mathbb{R}, \psi \in \Omega$. Dieser Fluss ist, als Abbildung von $\mathbb{R} \times \Omega$ nach $\Omega$ betrachtet, stetig differenzierbar. Damit ist auch $F_t:\Omega \to \Omega$ stetig differenzierbar $\forall t \in \mathbb{R}$. \\
Irgendwann Beweis, dazu brauchen wir: \\
\textbf{Einschub(01.12.25): Irgendwann ist jetzt!} \\
$F_0(\psi) = F(0,\psi) 0 x_\psi(0) = \psi$. \\
Wegen der Shiftinvarianz (siehe unterhalb) gilt: \\$(F_{t_2} \circ F_{t_1})(\psi) = F(t_2,(F(t_1,\psi)) = x_{F(t_1,\psi)}(t_2) = x_{x_\psi(t_1)}(t_2) = x_\psi(t_2+t_1) = F(t_2+t_1,\psi) = F_{t_2+t_1}(\psi)$ \\
Dies definiert einen Fluss. Die stetige Differenzierbarkeit folgt aus \\
$\frac{\partial}{\partial t}F(t,\psi) = f(F(t,\psi))$ und der Stetigkeit von $F$ in $t$ und in $\psi$. \\
Denn $F(t_2,\psi)-F(t_1,\psi) = \int_{t_1}^{t_2} f(x_\psi(t))dt \to 0$ für $t_2 \to t_1$. \\
Wir setzen voraus, dass Stetigkeit in $\psi$ gegeben ist. Dies kann man beweisen, Z.B. "Stetige Abhängigkeit von Anfangswerten" bei Differentialgleichungen. \\
Seien nun $\psi \in \Omega$ und $T>0$ beliebig, aber fest. Sei $t \in [0,T]$. Wegen der stetigen Abhängigkeit von den Anfangsdaten existiert eine kompakte Menge $K \subset \Omega$ so, dass $x_\psi(t),x_{\psi+h \mu}(t) \in K$ $\forall \mu \in \mathbb{R}^d$ mit $|\mu| = 1$, für alle $t \in [0,T]$ und alle genügend kleinen h. \\
Definiere $y_{h,\mu}(t) := \frac{F(t,\psi+h\mu) - F(t,\psi)}{h} = \frac{x_{\psi+h \mu}(t) - x_\psi(t)}{h}$. Wir wollen Konvergenz für $h \to 0$ zeigen. \\
Es gilt $y_{h,\mu}'(t) = \frac{f(x_{\psi+h \mu}(t))-f(x_\psi(t))}{h} = \frac{f(x_\psi(t) + h y_{h,\mu}(t))-f(x_\psi(t))}{h} \leq L |y_{h,\mu}(t)|$ mit $L = L(\psi, T, K)$. \\
$\Rightarrow |y_{h,\mu}|(t) \leq |y_{h,\mu}|(0) + \int_0^t L|y_{h,\mu}|(s)ds \underset{\text{Gronwall}}{\Rightarrow} |y_{h,\mu}(t)| \leq \exp(Lt)|\mu| \leq \exp(LT) |\mu|$. Schranke ist unabhängig von $h$. \\
Definiere nun $y_{0,\mu}$ als \Lsg des linearen, jedoch nicht autonomen AWP \\
$y'=Df(x_\psi(t))y, y(0) = \mu$ $(\star)$ \\
Also  $y_{0,\mu}(t) = \phi(t) \mu$, wobei $\phi(t)$ die Fundamentalmatrix von $(\star)$ ist $(\phi = \phi(x_\psi, \psi))$. \\
Damals: $x(t) = \phi(t) \phi^{-1}(t_0) \psi$. \\
Es gilt $y_{h,\mu}(t) - y_{0, \mu}(t) = \int_0^t Df(x_\psi(s)) (y_{h,\mu}8s) - y_{0,\mu}(s)) + R_{h, \mu}(s) ds$, wobei das Restglied \\
$R_{h,\mu}(t) := \frac{f(x_{\psi+h\mu}(t)) - f(x_\psi(t))}{h} - Df(x_\psi(s)) y_{h,\mu}(t)$ ist. \\ Gronwall liefert dann $|y_{h,\mu}(t)-y_{0,\mu}(t)| \leq a_{h,\mu} \exp(\int_0^t |Df(x_\psi(s))|ds)$, wobei $a_{h,\mu} := \int_0^T |R_{h,\mu}(s)|ds$ ist. \\
Da die Funktionen $y_{h,\mu}$ gleichmäßig in $h$ beschränkt sind, gilt $|R_{h,\mu}(t)| \leq C$ $\forall 0 \leq t \leq T, |\mu| \leq 1$ und alle $h$ klein genug. \\
Nach Taylor gilt Restgliedkonvergenz $\lim R_{h,\mu}(t) = 0$ $\forall t \in \mathbb{R}$ und alle $\mu$. \\
$\Rightarrow a_{h,\mu} \to 0, y_{h,\mu}(t) \to y_{0,\mu}(t) = \phi(t) \mu$ $\forall 0 \leq t \leq T$ \\
$\rightarrow$ Der Fluss besitzt im Punkt $(t,\psi)$ alle Richtungsableitungen in Richtung $\mu$ $\Rightarrow$ alle partiellen Ableitungen existieren $\frac{\partial}{\partial \psi_i} F(t,\psi), i=1,...,d$. Die Stetigkeit der Richtungsableitungen bezüglich $t$ und $\psi$ folgt, da $x_\psi$ stetig in $t$ und $\psi$ ist und da $\phi(t) \mu$ stetig von $t$ und $\mu$ abhängt. \\
\textbf{Ende Einschub} \\
\\
\textbf{Lemma: Shiftinvarianz von Lösungen autonomer \DGLs} \\
Seien $t_0, \overline{t_0} \in \mathbb{R}$. Sei $x:\mathbb{R} \to \Omega$ eine Lösung von $(\star)$ mit $x(t_0) = \psi$. \\
Dann ist auch $\tilde{x}:\mathbb{R} \to \Omega$ mit $\tilde{x}(t) = x(t-\tilde{t_0} + t_0)$ eine Lösung von $(\star)$ mit $\tilde{x}(\tilde{t_0}) = \psi$. \\
Insbesondere gilt $x(t) = x_\psi(t-t_0)$ und $\tilde{x}(t) = x_\psi(t-\tilde{t_0})$. \\
\\
\textbf{Definition} \\
Für $x'=A(t)x$ heißt eine Basis $x^{(1)}, x^{(2)}, ..., x^{(d)}: \mathbb{R} \to \mathbb{R}^d$ des Lösungsraumes Fundamentalsystem. Die Matrixwertige Fnktion $\phi:\mathbb{R} \to \{d \times d \text{ Matrizen}\}$ mit $\phi(t) = \begin{pmatrix}
x_1^{(1)}(t) & ... & x_1^{(d)}(t) \\
... & & ... \\
x_d^{(1)}(t) ... x_d^{(d)}(t)
\end{pmatrix}$ heißt Fundamentalmatrix und $det(\phi):\mathbb{R} \to \mathbb{R}$ heißt Wronski-Determinante. \\
Es gilt $\phi'=A(t) \phi$ und $x(t) = \phi(t) \phi^{-1}(t_0) \psi$ löst $x'=A(t)x$ mit $x(t_0) = \psi$. \\
\underline{Beispiel:} \\
$x'=\begin{pmatrix}
0 & 1 \\
-1 & 0
...
x_1 \\
x_2
\end{pmatrix}$ (harmonischer Oszillator). \\
Linear unabhängige \LSG sind $x^{(1)}(t) = \begin{pmatrix}
cos(t) \\
-sin(t)
\end{pmatrix}$ und $x^{(2)}(t) = \begin{pmatrix}
sin(t) \\
cos(t)
\end{pmatrix} \Rightarrow \phi(t) = \begin{pmatrix}
cos(t) & sin(t) \\
-sin(t) & cos(t)
\end{pmatrix}$ und $det(\phi(t)) = 1 \forall t \in \mathbb{R}$. \\
\underline{Bemerkung:}
\begin{itemize}
\item Da $(F_t)^{-1} = F_{-t}$, ist jede Abbildung $F_t:\Omega \to \Omega$ ein Diffeomorphismus.
\item Für $f(x) = Ax$ ist $F_t(\psi) = exp(tA) \psi$.
\end{itemize}
\underline{Das Theorem von Poincare-Deudixon:}

\subsection{Annahmen}
Sei $f$ stetig diffbar und auf ganz $\mathbb{R}^2$ definiert. Existiere für alle $\psi \in \mathbb{R}$ die \Lsg $x_\psi \forall t \in \mathbb{R}$ für unsere \DGL.

\subsection{Definition}
Gelte 12.8. Für jedes $\psi \in \mathbb{R}^2$ heißen $\alpha_\psi := \{ \psi \in \mathbb{R}^2 | \text{Es existiert eine Folge } \{t_n \}_{n \in \mathbb{N}}, t_n \to - \infty, x_\psi(t_n) \to \psi \}$, $\omega\psi := \{ \psi \in \mathbb{R}^2 | \text{Es existiert eine Folge } \{t_n \}_{n \in \mathbb{N}}, t_n \to + \infty, x_\psi(t_n) \to \psi \}$ die $\alpha$-Limes- und die $\omega$-Limesmenge von $\psi$.

\subsection{Lemma}
Gelte 12.8. Für jedes $\psi$ ist $\omega_\psi$ abgeschlossen und invariant unter der Dynamik, d.h. $T_\psi \subset \omega_\psi \forall \psi \in \omega_\psi$ und damit $x_\psi(t) \in \omega_\psi \forall \psi \in \omega_\psi$. Analog für $\alpha_\psi$.

\subsection{Theorem: Poincare-Beudixson}
Gelte 12.8. Sei $\underset{t \to \infty}{\limsup} |x_\psi(t)| < \infty$, dann enthält $\omega_\psi$ entweder einen Fixpunkt oder ist ein echt-periodischer Orbit.

\subsection{Folgerung}
Gelte 12.8. Sei $\Sigma \in \mathbb{R}^2$ ein beschränktes Gebiet. Sei $\overline{\Sigma}$ vorwärts-invariant unter der Dynamik, d.h. $x_\psi(t) \in \overline{\Sigma}$ $\forall \psi \in \overline{\Sigma}, t \geq 0$. \underline{Dann} enthält $\overline{\Sigma}$ einen stationären Punkt oder eonen echt-periodischen Orbit.

\subsection{Lemma: Beudixon-kriterium}
gelte 12.8. Eine einfach zusammenhängende Menge $\Sigma \in \mathbb{R}^2$ kann nur dann einen echt-periodischen Orbit enthalten, wenn $div f$ mindestens eine Nullstelle in $\Sigma$ besitzt.

\subsection{Beispiel}
Betrachte $x'=\begin{pmatrix}
-x_1+x_2^2 \\
x_1^2 - x_2^3
\end{pmatrix}$. Dann $div f(x_1,x_2) = -1-3 x_2^2$. Also existiert \underline{keine} Nullstelle(weil $div f$ keine Nullstelle in $\mathbb{R}$ besitzt ?). \\
\\
\underline{\textbf{Stabilität von stationären Punkten}} \\
Wir betrachten stationäre Orbits von $x'=f(x)$, d.h. konstante \LSG $x_{\psi^\star}(t) = \psi^\star$ $\forall t$. \\
Wann sind diese stabil unter kleinen Störungen? \\
Wir wissen, dass $T_{\psi^\star} = \{\psi^\star\}$ genau dann, wenn $\psi^\star$ kritischer Punkt des Vektorfeldes $f$ ist, d.h. gaenau dann, wenn $f(\psi^\star) = 0$ ist.

\subsection{Definition: Lyapunov-Stabilität}
Gelte 12.8. Ein stationärer Punkt $\psi^\star \in \Omega$ (bzw. der Orbit $T_{\psi^\star}$, bzw. die stationäre \Lsg $x_{\psi^\star}$) heißt
\begin{itemize}
\item Lyapunov-stabil(oder einfach stabil), falls $\forall \epsilon > 0$ ein $\delta > 0$ existiert, sodass für $|\psi - \psi^\star| \leq \delta \Rightarrow |x_{\psi}(t) - \psi^\star| \leq \epsilon \forall t \geq 0$.
\item asymptotisch stabil, falls Stabilität gilt und ein $\tilde{\delta}$ existiert, sodass für $|\psi - \psi^\star| \leq \tilde{\delta} \Rightarrow \underset{t \to \infty}{\lim} x_\psi(t) = \psi^\star$. Ein nicht-stabiler stationärer Punkt heißt instabil.
\end{itemize}

\subsection{Linearisierte Stabilität}
Gelte 12.8. Sei $\psi^\star$ ein stationärer Punkt und haben alle (komplexen) Eigenwerte (also auch reelle) der Matrix $A:=Df(\psi^\star)$ alle negativen Realteil haben, d.h.$-\mu = max\{Re(\lambda) | \lambda \in \text{Spektrum } A \} < 0$.\\
Dann existieren für jedes $0 < \alpha < \mu$ Konstanten $C>0, \delta > 0$, sodass \\
$|x_\psi(t) - \psi^\star| \leq C exp(-\alpha t)|\psi - \psi^\star|$ $\forall t$ und alle $\psi$ mit $|\psi - \psi^\star| \leq \delta$. \\
$\Rightarrow \psi^\star$ ist asymptotisch stabil.

\subsection{Theorem: Grobmann und Hartmann}
Gelte 12.8. Sei der stationäre Punkt $\psi^\star$ \underline{hyperbolisch}, d.h. alle (komplexen) Eigenwerte von $A:=Df(\psi^\star)$ haben nicht-verschwindenden Realteil. \underline{Dann} existieren zwei Umgebungen $U$ und $V$ von $\psi^\star$ sowie ein Homöomorphismus $H:U \to V$, sodass $H(U \cap V) = \square_{H(\psi)} \cap V$ gilt, wobei $\square_\psi$ die Orbits der \underline{linearisierten} Gleichung $y'=Df(\psi^\star)(y-\psi^\star)$ bezeichnet. \\
Das heißt unter der Hyperbolitätsannahme beschreibt der Fluss der linearisierten Gleichung lokal den Fluss der nicht-linearen Gleichung.
...
-x_1 x_2 \\
-x_2 + x_1^2 - 2 x_2^2
\end{pmatrix}$. Stationäre Punkte sind $\mu^\star = \begin{pmatrix}
0 \\
0
...
-\frac{1}{2}
\end{pmatrix}$. \\
$A := Df(\mu^\star) = \begin{pmatrix}
-x_2 & -x_1 \\
2x_1 & -1-4x_2
\end{pmatrix} |_{\mu^\star} = \begin{pmatrix}
0 & 0 \\
0 & -1
\end{pmatrix} \rightarrow$ Eigenwerte 0 und -1. \\
$B := Df(\nu^\star) = \begin{pmatrix}
-x_2 & -x_1 \\
2x_1 & -1-4x_2
\end{pmatrix} |_{\nu^\star} = \begin{pmatrix}
\frac{1}{2} & 0 \\
0 & 1
\end{pmatrix} \rightarrow$ Eigenwerte $\frac{1}{2}$ und 1. \\
\item Betrachte $x'=\begin{pmatrix}
-x_1+x_2 \\
2-x_1^2-x_2^2
\end{pmatrix}$. Stationäre Punkte: $\mu^\star = \begin{pmatrix}
1 \\
1
\end{pmatrix}, \nu^\star = \begin{pmatrix}
-1 \\
-1
\end{pmatrix}$. \\
Nun Linearisieren wir, indem wir die Jacobi-Matrix bestimmen und an $\mu_\star$ und $\nu_\star$ auswerten: \\
$A := Df(\mu^\star) = \begin{pmatrix}
-1 & 1 \\
-2x_1 & -2x_2
\end{pmatrix} |_{\mu^\star} = \begin{pmatrix}
-1 & 1 \\
-2 & -2
\end{pmatrix}$ \\ Für den Punkt $\mu_\star$ betrachten wir also das lineare System $x'=Ax$ \\
$\rightarrow$ Eigenwerte(über char. Polynom $det(A-\lambda I_d)$): $(-1-\lambda)(-2-\lambda)+2 = \lambda^2 + 3 \lambda + 4 = 0 \Rightarrow \lambda_{1,2} = -\frac{3}{2} + - \sqrt{\frac{9}{4} - \frac{16}{4}} = -\frac{3}{2} + - i \sqrt{7} \Rightarrow $ stabiler Wirbel(auch wahr für das nichtlineare System). \\
$B := Df(\nu^\star) = \begin{pmatrix}
-1 & 1 \\
2 & 2
\end{pmatrix}$ \\ Für den Punkt $\nu_\star$ betrachten wir also das lineare System $x'=Bx$ \\
$\rightarrow$ Eigenwerte(über char. Polynom $det(B-\lambda I_d)$): $(-1-\lambda)(2-\lambda)-2 = \lambda^2 - \lambda - 4 = 0 \Rightarrow \lambda_{1,2} = \frac{1}{2} + - \frac{\sqrt{17}}{2} \Rightarrow$ Sattelpunkt. goodnotes 11
\end{enumerate}
Wir können immer annehmen $\psi^\star = 0$, denn: \\ Für $x'=Ax$ klar. \\
Wenn $x'=f(x), f(\psi^\star) = 0$. Setze $y=x-\psi^\star \Rightarrow y'=x'=f(x) = f(y+\psi^\star$. \\
$f(\psi^\star + y) = f(\psi^\star) = f(\psi^\star+y)$. Setze $g(y) = f(\psi^\star+y), g'(0) = \frac{df(\psi^\star + y)}{dy}|_{y=0} = f'(\psi^\star)$. \\
Die Systeme 
$y'=g(y), y=0, y'=g(0) y$ \\
$x'=f(x), x = \psi^\star, x'=f'(\psi^\star)x$
sind äquivalent.

\subsection{Definition: Nichtlineare Stabilitätskonzepte}
Sei $\psi^\star$ ein stationärer Punkt einer DGL. Eine Funktion $L \in C^1(\Omega), L: \Omega \to \mathbb{R}$ heißt Lyapunov-Funktion von $\psi^\star$, wenn:
\begin{enumerate}
\item $L(\psi^\star) < L(x) \forall x \in \Omega \setminus \{\psi^\star\}$
\item $L'(x) = DL(x)f(x) \leq 0 \forall x \in \Omega \setminus \{\psi^\star\}$ \\
Falls $L'(x) < 0$, heißt $L$ strikte Lyapunovfunktion.
\end{enumerate}

\subsection{Theorem}
Jeder stationäre Punkt $\psi^\star$, für den es eine (strikte) Lyapunov-funktion existiert, is (asymptotisch) stabil.

\subsection{Beispiel: Differentialgleichung zweiter Ordnung}
\begin{itemize}
\item $y''+g(y)y'+y = 0, g \geq 0$, stetig \\
Transformiere in ein System erster Ordnung: $x_1=y, x_2 = y'$ \\
$x_2'+g(x_1)x_2 + x_1 = 0 \Rightarrow x_2'=-g(x_1)x_2-x_1$ \\
$x_1'=x_2$ \\
$\begin{pmatrix}
x_1 \\
x_2
...
x_2 \\
-g(x_1)x_2 - x_1
\end{pmatrix} := f(x_1,x_2)$ \\
Finde stationäre Punkte: \\
$x_2 = 0, g(x_1)x_2 - x_1 = 0 \Rightarrow x_1 = 0$ \\
Also ist $\begin{pmatrix}
0 \\
0
\end{pmatrix}$ der einzige stationäre Punkt. \\
$L(x_1,x_2) = x_1^2 + x_2^2 \geq 0 = L(0,0)$ \\
$L(x_1,x_2) > L(0,0)$ für $(x_1,x_2) \neq (0,0)$ \\
$L'(x_1,x_2) = \nabla L (x_1',x_2') = (2x_1, 2x_2)(x_2, -g(x_1)x_2-x_1) \\ = 2 x_1x_2 - 2 x_2g(x_1)x_2-2x_1x_2 = -2x_2^2g(x_1) \leq 0$ \\
Also ist $\begin{pmatrix}
0 \\
0
\end{pmatrix}$ stabil.
\item Volterra-Lotka (x: Beute, y: Jäger): \\
$x'(t) = ax(t) - \alpha x(t)y(t)$ \\
$y'(t) = -c y(t) + \gamma x(t) y(t)$ \\
$ax-\alpha xy = 0 \Rightarrow x(a-\alpha y) = 0 \Rightarrow x=0 \lor y = \frac{a}{/alpha}$ \\
$-cy+\gamma xy = 0 \Rightarrow y(-c+\gamma x) = 0 \Rightarrow y=0 \lor x = \frac{c}{\gamma}$ \\
$\Rightarrow (0,0), (\frac{c}{\gamma}, \frac{a}{\alpha})$ \\
$f(x,y) = \begin{pmatrix}
ax-\alpha xy \\
-cy + \gamma xy
\end{pmatrix} \Rightarrow Df = \begin{pmatrix}
a-\alpha y & -\alpha x \\
\gamma y & -c+\gamma x
\end{pmatrix}$ \\
Bei $(0,0)$: $Df(0,0) = \begin{pmatrix}
a & 0 \\
0 & -c
\end{pmatrix} \Rightarrow$ Eigenwerte: $a, -c. \Rightarrow$ Sattelpunkt. \\
Bei $(\frac{c}{\gamma}, \frac{a}{\alpha})$: $Df(\frac{c}{\gamma}, \frac{a}{\alpha}) = \begin{pmatrix}
0 & \alpha \frac{c}{\gamma} \\
\gamma \frac{a}{\alpha} & 0
\end{pmatrix} \Rightarrow$ Eigenwerte: $\sqrt{\alpha c} i, - \sqrt{\alpha c} i$. Man kann nicht auf das nichtlineare System schließen. \\
$L(x,y) = \gamma x - c \cdot log(x) + \alpha y - a \cdot log(y) > L(\frac{c}{\gamma}, \frac{a}{\alpha})$ \\
$f(x) = \gamma x - c \cdot log(x) \Rightarrow f'(x) = \gamma - \frac{c}{x}$ Minimum bei $x=\frac{c}{\gamma}$ \\
$g(y) = \alpha y - a \cdot log(y) \Rightarrow g'(y) = \alpha - \frac{a}{y}$  Minimum bei $y = \frac{a}{\alpha}$\\
$\nabla L \cdot f \leq 0$ \\
$\frac{d L(x(t),y(t))}{dt} \leq 0$ \\
$\nabla L = \begin{pmatrix}
\gamma - \frac{c}{x} \\
\alpha - \frac{a}{\gamma}
\end{pmatrix}$ \\
$\nabla L \cdot f = (\gamma - \frac{c}{x}, \alpha . \frac{\alpha}{y})\cdot(ax-\alpha xy , -cy + \gamma xy) = \gamma ax - \gamma a x y - \frac{c}{x} ax + \frac{c}{x} axy - cy \alpha + ??wtf??? = 0$ \\
Lyapunov-stabil \\
\underline{Erinnerung:} In 11.1 war das Räuber-Beute-Modell $x'=(\alpha - \beta y)x, y'=(-\gamma + \delta x)y$.
\item $u_t = u_{xx} + u(1-u)$ \\
$u(t,x) = U(x-ct)$ \\
goodnotes 12 \\
$u_t = -c U'(x-ct), u_{xx} = U''(x-ct)$ \\
$-cU'=U''+U(1-U)$ \\
$-cU'=U''+U(1-U), U(-\infty) = 1, U(\infty) = 0$ \\
Mit der Zeit verschieben sich die Wellen. \\
Durch Transformation: $\begin{pmatrix}
U \\
V
\end{pmatrix}' = \begin{pmatrix}
V \\
-cV-U(1-U)
\end{pmatrix}$, stationäre Punkte $(0,0), (1,0)$. \\
...
0 & 1 \\
-1 & -c
\end{pmatrix} \Rightarrow$ Eigenwerte über char. Polynom: $\lambda_{1,2} = \frac{-c + - \sqrt{c^2-4}}{2}$, beide negativ. Stabiler Knoten für $c \geq 2$, stabiler Wirbel für $c < 2$. \\
$Df(U,V) = \begin{pmatrix}
0 & 1 \\
2U-1 & -c
\end{pmatrix}$ \\
...
0 & 1 \\
1 & -c
\end{pmatrix} \Rightarrow$ Eigenwerte über char. Polynom: $\lambda_{1,2} = \frac{-c + - \sqrt{c^2+4}}{2}$, einer negativ, einer positiv, beide reell. Sattelpunkt.
\end{itemize}
01.12.25:
\subsection{Definition: Erhaltungsgröße}
(Anderer Name für 12.19) \\
Gelte 12.8. $E:\Omega \to \mathbb{R}$ stetig differenzierbar heißt Erhaltungsgröße (oder erstes Integral) unserer Differentialgleichung, falls $d_f E(x) := DE(x)f(x) = 0$ $\forall x \in \Omega$ ist.

\subsection{Theorem}
(Anderer Name für 12.20) \\
Gelte 12.8. Sei $E$ eine zweimal stetig differenzierbare Erhaltungsgröße und $\xi_\star$ ein striktes lokales Minimum/Maximum von $E$. Dann ist $\xi_\star$ ein stabiler stationärer Punkt.

\subsection{Bezug auf Kapitel 10: SIR-Modell}
$s' = -\alpha is, i'= \alpha is - \beta i, r'= \beta i$ \\
$\underbrace{(s+i+r)'}_{\text{Invariante}}=0$, $\underbrace{(log(s)-\frac{\alpha}{\beta}s-\frac{\alpha}{\beta}i)'}_{\text{Invariante}}=0$ \\
$\beta \log(s(\infty)) - \alpha s(\infty) - \alpha i(\infty) = \beta log(s(0))-\alpha s(0)-\alpha i(0)$ \\
Wie würde man unter der Voraussetzung, dass dieses Modell die Realität gut approximiert, in das epidemische Geschehen eingreifen? \\
Sei $i(0)$ klein und $s(0) > \frac{\beta}{\alpha}$. \underline{Ziel:} Senkung der Infektionsrate $\alpha$ auf $\tilde{\alpha}$ durch Maßnahmen. \\
\underline{Frage:} Wie weit sollte man $\alpha$ senken? Final ist ohnehin $s(\infty) \leq \frac{\beta}{\alpha}$. Man möchte $s(\infty) = \frac{\beta}{\alpha}$ erreichen, aber für das $\tilde{\alpha}$-Modell.\\
$\beta \log(s(\infty)) - \tilde{\alpha} s(\infty) - \underbrace{\tilde{\alpha} i(\infty)}_{=0} = \beta log(s(0))-\tilde{\alpha} s(0) -\tilde{\alpha} i(0)$ \\
$\beta \log \frac{\beta}{\alpha} - \tilde{\alpha} \frac{\beta}{\alpha} = \beta \log s(0) - \tilde{\alpha} (s(0)+i(0))$ bzw. $\alpha \beta (\log \beta - \log \alpha - \log s(0)) = \tilde{\alpha}(\beta - \alpha(s(0)+i(0)))$ \\
$\Rightarrow \tilde{\alpha} = \frac{\alpha \beta (\log \beta - \log \alpha - \log s(0))}{\beta - \alpha(s(0)+i(0))}$ wäre in diesem Sinne optimal.

\section{Numerische Methoden für \DGLs}
04.12.25: \\
Orientierung an Buch: Rolf-Rannackv, Numerik gewöhnlicher Differentialgleichungen \\

\subsection{}
Betrachte $u: I:=[t_0, t_0+T] \to \mathbb{R}$ und $u'(t) = f(t,u(t)), t \geq 0$ mit $u(t_0)=u_0$. \\
Dazu gehört die Integralgleichung (/Fixpunktgleichung): \\
$u(t) = u_0 + \int_0^t f(s,u(s)) ds$ für $t \in I$

\subsection{Fixpunktiteration}
Gelte 13.1. Sei $u^0 \equiv u_0$ der Startwert. Für $k \geq 1$ definiere: \\
$u^k(t) = u_0 + \int_{t_0}^t f(s,u^{k-1}(s)) ds$ \\
Hier ist $u^i$ keine Potenz, sondern $i$ ist ein Index. \\
Die Konvergenz dieser Iteration wird durch den Banachschen Fixpunktsatz sichergestellt, \underline{falls} die Abbildung \\
$g(v)(t) = u_0 + \int_{t_0}^t f(s,v(s)) ds$ eine Kontraktion im Banachraum $C(I)$ ist. Falls $f(t,\cdot)$ Lipschitz-stetig ist, dann gilt: \\
$\underset{I}{\max} |g(v)-g(w)| \leq \int_{t_0}^t \underset{I}{\max} |f(s,v)-f(s,w)| ds \leq L \underset{I}{\max} |v-w|$ und wir haben eine Kontraktion für $T < \frac{1}{L}$.

\subsection{Methode der Taylorentwicklung}
Gelte 13.1. Sei die Lösung $u$ des Anfangswertproblems analytisch. \\
Dann lässt sich $u(t_0+T) = \sum_{k=0}^\infty \frac{T^K}{k!} u^{(k)}(t_0) = u_0 + T \sum_{k=1}^\infty \frac{T^{k-1}}{k!} f^{(k-1)}(t_0, u_0)$ in eine Taylorreihe entwickeln, wobei $f^{(k)}(t,x)$ die k-te \underline{totale} Zeitableitung von $f(t,u(t))$ ist. \\
Also ist $f^{(1)}(t,x) = (f_t'+f_x'f)(t,x)$. (Da x Variable ist, kann man z.B. auch $u$ statt $x$ schreiben.) \\
Durch Abschneiden der Reihe bei $k=m$ erhält man ein numerisches Verfahren. \\
Dieses Verfahren ist meist $\frac{\text{"zu teuer"}}{\text{nicht praktikabel}}$.

\subsection{Finite Differenzen}
Gelte 13.1. Betrachte das Punktgitter (Menge an Zeitpunkten) des Intervalls $I$, $t_0 < t_1 < ... < t_n < ... < t_N = t_0+T$ mit Gitterweiten $h_n = t_n - t_{n-1}$.\\
Berechne $y_n = y_{n-1} + h_n f(t_{n-1}, y_{n-1}), n \geq 1, y_0 = u_0$ (Eulersche Polygonzugmethode \underline{oder} explizites Euler-Schema) \\
Hierdurch wird ein approximierender Polygonzug erzeugt: \\
$y(t) := y_{n-1} + (t-t_{n-1})f(t_{n-1}, y_{n-1}), t \in [t_{n-1}, t_n]$ \\
(Hier ist glaube ich $y_{n} = y(t_n)$) \\
Der neue Wert $y_n$ wird aus dem alten Wert $y_{n-1}$ berechnet durch Auswerten von $f(t_{n-1}, y_{n-1})$. Dies ist ein \underline{explizites} Verfahren. Das analoge \underline{implizite} Verfahren (implizites Eulerschema) erhält man durch $y_n - h_n f(t_n, y_n) = y_{n-1}$. \\
Hier muss für die Berechnung von $y_n$ aus $y_{n-1}$ ein (im Allgemeinen) \underline{nicht lineares} Gleichungssystem gelöst werden. Für alle Verfahren sind Konvergenzabschätzungen nötig. Für Euler \underline{kann} man Konvergenzabschätzungen der Form $\underset{t_n \in I}{\max} |y_n - u(t_n)| \leq c(T, u) h$, wobei $h := \underset{n}{\max} h_n$, $c$ ist eine Konstante! \\
Implizite verfahren sind sehr aufwändig und auch nur in speziellen Situationen (steife Probleme) relevant, wenn explizite Verfahren aus Gründen  der numerischen Stabilität \underline{nicht} verwendet werden können.

\subsection{Galerkin Methode}
Gelte 13.1. Wir formulieren das AWP "variationell". \\
Wir multiplizieren die \DGL mit einer sogenannten Testfunktion $\phi$ \\
$\int_I u'(t) \phi(t) ds = \int_I f(t,u(t)) \phi(t) dt$. Dies kann man sinnvoll für jede stetige und stückweise stetig differenzierbare Funktion $u$ formulieren. Sei $V$ der Vektorraum all dieser Funktionen. Das linke Integral kann also auch stückweise sein, d.h. eine Summe von Teilintegralen. Die Galerkin Methode bestimmt eine Näherungslösung $u_h$ in einem Teilraum $V_h \subset V$, dem sogenannten Ansatzraum mit dem Startwert $u_h(t_0) = u_0$ und  \\
$\int_I u_h'(t) \phi_h(t) dt = \int_I f(t,u_h) \phi_h(t) dt$ für ein beliebiges $\phi_h \in W_h$, wobei $W_h$ ein diskreter Testfunktionenraum ist, in der Regel verschieden von $V_h$. \\
\textbf{Beispiel:} Zerlege $I$ gemäß $t_0 < t_1 < ... < t_n < ... < t_N = t_0+T$. \\
Setze $V_h := \{ v_h : I \to \mathbb{R}|v_h \in C(I), v_h|_{(t_{n-1}, t_n]} \in P_1 \text{ für } n=1,...,N \}$, wobei $P_1$ die Menge der Polynome ersten Grades ist. \\
Setze $W_h := \{ \phi_h:T \to \mathbb{R}| \phi_h|_{(t_{n-1}, t_n]} \in P_0 \text{ für } n=1,...,N \}$. \\
Relevant wird am Montag: $u_h(t_n) - u_h(t_{n-1}) = \int_{t_{n-1}}^{t_n} u_h'(t) dt = \int_{t_{n-1}}^{t_n} f(t,u_h(t)) dt$ \\
08.12.25: \\
Da die Testfunktionen nur stückweise stetig zu sein brachen, kann die Bestimmungsgleichung (die hier drüber) auf jedes Teilintervall eingeschränkt werden. \\
Das Galerkin-Verfahren ist, wie das Differenzenverfahren, ein sogenanntes "Zeitschrittverfahren" (besteht aus einzelnen Zeitschritten). Werte das rechte Integral mit Hilfe der Trapezregel aus: \\
$y_n - y_{n-1} = \frac{1}{2} h_n (f(t_n, y_n) + f(t_{n-1}, y_{n-1}))$ für $y_n := u_h(t_n)$. \\
Dies ist ein implizites Differenzenverfahren. Man \underline{kann} zeigen, dass die Konvergenzabschätzung $\underset{t \in I}{\max} |u_h(t) - u(t)| \leq c(T,u) h^2$ gilt für $h:=\underset{n}{\max}$ $h_n$. \\
Wir erhalten also eine höhere Genauigkeit als beim impliziten Eulerverfahren.

\subsection{Verfahrensanalyse}
Wie kann man die Effizienz/Güte verschiedener Lösungsverfahren beurteilen? \\
Dazu gehören Fragen der Konvergenz, die Konvergenzordnung, der numerischen Stabilität, eine zuverlässige Kontrolle des Approximationsfehlers und eine effektive Schrittweitensteuerung. \\

\subsection{Analyse der Eulerschen Polygonzugmethode}
Betrachte das Anfangswertproblem $u'(t)=f(t,u(t)), t \in I = [t_0, t_0+T], u(t_0) = u_0$. \\
Sei $f(t,x)$ stetig auf $I\times\mathbb{R}^d$ und genüge einer globalen Lipschitzbedingung, sodass $||f(t,x)-f(t,y)|| \leq L_f ||x-y||$ für $(t,x), (t,y) \in I\times\mathbb{R}^d$. Dann existiert ein eindeutig bestimmtes $u(t)$ für unser Anfangswertproblem $\forall t \in I$ und \\
$||f(t,x)|| \leq ||f(t,x)-f(t,0)|| + ||f(t,0)|| \leq L_f ||x|| + ||f(t,0)||$. \\
Sei diese Lösung hinreichend glatt. \\
Wie zuvor wählen wir eine Folge von diskreten Zeitpunkten $t_0 < ... < t_n < ... t_N = t_0+T$. Definiere $I_n := [t_{n-1}, t_n]$, $h_n := t_n - t_{n-1}, h := \underset{1 \leq n \leq N}{\max}$ $h_n$. \\
Sei $y_0^h \in \mathbb{R}^d$ der Startwert und betrachte die rekursive Vorschrift $y_n^h = y_{n-1}^h + h_n f(t_{n-1}, y_{n-1}^h), n=1,...,N$. \\
Wir schreiben dies in Form einer Differenzengleichung: \\
$(L_h y^h)_n = 0, n=1,...,N$ mit dem Differenzenoperator $(L_hy^h)_n := h^{-1}_n (y_n^h - y_{n-1}^h) - f(t_{n-1}, y_{n-1}^h)$ für die Gitterfunktion $y^h = \{ y_n^h \}_{n=1,...,N}$. \\
Man \underline{kann} zeigen, dass falls $y_0^h \to u_0$ konvergiert für $h \to 0$, dann $\underset{0 \leq n \leq N}{\max}$ $||y_n^h - u(t_n)|| \to 0$ für $h \to 0$, wegen der Eindeutigkeit der Lösung $u$. \\
\textbf{Konvergenzgeschwindigkeit:} Definiere den Abschneidefehler \\
$\tau_n^h := (L_h u^h)_n = h_n^{-1} (u_n^h - u_{n-1}^h) - f(t_{n-1}, u_{n-1}^h)$ für die Gitterfunktion \\
$u^h := (u_n^h := u(t_n))_{0 \leq n \leq N}$. Da $u'(t) = f(t,u)$ ist, gilt $\tau_n^h = h_n^{-1} \int_{t_{n-1}}^{t_n} u'(t) dt - u'(t_{n-1}) \overset{\text{Produktregel}}{=} h_n^{-1} \int_{t_{n-1}}^{t_n} (t_n-t) u''(t) dt$ \\
$\Rightarrow ||\tau_n^h|| \leq \frac{1}{2} h_n \underset{t \in I_n}{\max}$ $||u''(t)||$ $(\star)$. \\
Dies ist eine Diskretisierung erster Ordnung. \\
Falls keine Missverständnisse entstehen, schreiben wir im Folgenden auch $y_n$ für $y_n^h$ und $\tau_n = \tau_n^h$(für u auch). Dann gilt $u_n = u_{n-1} + h_n f(t_{n-1}, u_{n-1}) + h_n \tau_n$. Die Abschätzung des globalen Diskretisierungsfehlers $e_n = y_n - u_n$ erfolgt über einen sogenannten Stabilitätssatz für Differenzenverfahren. \\
$e_n = e_{n-1} + h_n (f(t_{n-1}, y_{n-1}) f f(t_{n-1}, u_{n-1})) - h_n \tau_n$. Da $f(t,x)$ Lipschitz-stetig ist, gilt: \\
$||e_n|| \leq ||e_{n-1}|| + h_n L_f ||e_{n-1}|| + h_n ||\tau_n||$. Durch sukzessive Anwendung dieser Ungleichung erhalten wir: $||e_n|| \leq ||e_0|| + L_f \sum_{\nu = 0}^{n-1} h_{\nu+1} ||e_\nu|| + \sum_\nu^n h_\nu ||\tau_\nu||$. \\
Nun benötigen wir den \\
\textbf{Hilfssatz: Diskretes Lemma von Gronwall} \\
Seien $(w_n)_{n \geq 0}, (a_n)_{n \geq 0}, (b_n)_{n \geq 0}$ Folgen nicht-negativer Zahlen mit $w_0 \leq b_0$ und \\
$w_n \leq \sum_{\nu = 0}^{n-1} a_\nu w_\nu + b_n$, $n \geq 1$. \\
Falls $(b_n)_{n \geq 0}$ monoton steigend ist, dann gilt $w_n \leq exp(\sum_{\nu = 0}^{n-1} a_\nu) b_n, n \geq 1$. \\
Mit diesem diskreten Lemma von Gronwall folgern wir, dass: \\
$||e_n|| \leq e^{L_f(t-t_0)} (||e_0|| + \sum_{\nu = 1}^n h_\nu ||\tau_\nu||), n \geq 1$ bzw. \\
$\underset{1 \leq n \leq N}{\max}$ $||e_n|| \leq e^{L_fT} (||e_0|| + T \underset{1 \leq n \leq N}{\max}$ $||\tau_n||)$. \\
$\Rightarrow \underset{1 \leq n \leq N}{\max}$ $||e_n|| \leq e^{L_fT}(||e_0|| + \frac{1}{2} T \underset{1 \leq n \leq N}{\max} \{ h_n \underset{t \in I_n}{\max}$ $||u''(t)|| \})$. \\
Damit ist die globale Konvergenzordnung mindestens gleich der lokalen Konsistenzordnung (wird später erklärt). \\
Wegen ihrer geringen Genauigkeit hat die Eulersche Polygonzugmethode in der Praxis keine Bedeutung.

\subsection{Allgemeine Einschrittmethoden}
Für Differenzenformeln höherer Ordnung betrachten wir die Taylorentwicklung. \\
Sei $d=1$. Dann ist $u(t) = \sum_{r=0}^R \frac{h^r}{r!} u^{(r)}(t-h) + \frac{h^{R+1}}{(R+1)!} u^{(R+1)} (\xi)$ für $\xi \in [t-h, t]$. \\
11.12.25: \\
Da $u'=f(t,u)$ gilt, dass $u^{(r)}=(\frac{d}{t})^{(r-1)} f(t,u(t)) =: f^{(r-1)} (t,u(t))$. Damit lautet das "R-stufige Taylor-Verfahren" dann \\
$y_n = y_{n-1} + h \sum_{r=1}^R \frac{h^{r-1}}{r!} f^{(r-1)} (t_{n-1}, y_{n-1}), n \geq 1$. Nun schreiben wir die allgemeine Formel für ein "Einschrittverfahren" wie folgt: \\
$y_n = y_{n-1} + h_n F(h_n, t_{n-1}, y_n, y_{n-1})$ bzw. $(L_hy)_n = h_n^{-1}(y_n-y_{n-1}) - F(h_n, t_{n-1}, y_n, y_{n-1}) = 0 (\star)$ mit der sogenannten Verfahrensfunktion $F(h,t,y,x) := \sum_{r=1}^R \frac{h^{r-1}}{r!} f^{(r-1)}(t,x)$. \\
Da die Verfahrensfunktion \underline{nur} von $y_{n-1}$, also der unmittelbar vorausgehenden Näherung, abhängt, heißt diese Methode explizit: $F=F(h_n, t_{n-1},y_{n-1})$ (also nicht von $y_n$ abhängig). \\
Der sogenannte \underline{Abschneidefehler} ist definiert durch \\
$\tau_n := (L_hu)_n = h_n^{-1}(u_n-u_{n-1}) - F(h_n, t_{n-1},u_n, u_{n-1})$ \\

\subsection{Definition: Konsistenz}
Die Einschrittmethode $(\star)$ heißt \underline{konsistent} mit unserem AWP \underline{bzw. konsistent mit Konsistenzordnung m}, falls $\underset{t_n \in I}{\max} ||\tau_n|| \to 0$ bzw. $\underset{t_n \in I}{\max} ||\tau_n|| = O(h^m)$ für $h \to 0$. \\
Die R-stufige Taylorformel für skalare AWP hat damit die Konsistenzordnung $m=R$. \\
Sei $f_t := \frac{\partial f}{\partial t}$ und $f_x:=\frac{\partial f}{\partial x}$. Totale Ableitung $f^{(1)}(t,x) = (f_t + f_x f)(t,x)$, wobei $f=x'(t)$. \\
$f^{(2)}(t,x) = (f_{tt} + 2 f_{tx} f + f_xf_t + f_{xx}f^2 + f_x^2f)$. (soll in der Übungsaufgabe begründet werden) \\
Dies kann in der Praxis sehr aufwendig sein. Ersetze die Ableitungen $f^{(r-1)}$ durch Differenzenquotienten. \\
\underline{z.B. für R=2} $f^{(1)}(t,u(t)) \sim h^{-1} (f(t+h, u(t+h)) - f(t,u(t))) \sim h^{-1} (f(t+h, u(t) + h f(t,u(t))) - f(t,u(t)))$ \\
$\Rightarrow y_n = y_{n-1} + h_nf(t_{n-1}, y_{n-1}) + \frac{1}{2} h_n ( f(t_n, y_{n-1} + h_m f(t_{n-1}, y_{n-1})) - f(t_{n-1}, y_{n-1}) ) \\
= y_{n-1} + h_n ( \frac{1}{2}f(t_{n-1}, y_{n-1}) + \frac{1}{2} f(t_n, y_{n-1 + h_n f(t_{n-1}, y_{n-1})}) )$ \\
Wenn man die Restglieder hier mitverfolgt, dann ergibt sich die Konsistenzordnung $m=2$. \\
Damit haben wir allgemein die sogenannten (expliziten) Runge-Kutta-Verfahren der Form \\
$f(h,t,x) = \sum_{r=1}^R c_rk_r(h,t,x)$ mit $k_1 = f(t,x), k_r = f(t+ha_r, x+h \sum_{s=1}^{r-1} b_{rs} k_s), r=2,...,R$ erhalten, mit geeignet gewählten Konstanten $a_r, c_r, b_{rs}$. Diese Konstanten bestimmt man so, dass für möglichst großes $m$ (idealerweise $m=R$) gilt: \\
$\sum_{r=1}^R c_rk_r(h,t,u(t)) = \sum_{r=1}^m \frac{h^{r-1}}{r!} f^{(r-1)} (t,u(t)) + O(h^m)$. \\
Konstruktionsgemäß ist damit die Konsistenzordnung der entsprechenden Runge-Kutta-Formel genau $m$.

\subsection{Beispiel (für Einschrittverfahren): Runge-Kutta-Methode der Stufen $R=1,2,3,4$:}
\begin{itemize}
\item R=1: Eulersche Polygonzugmethode

\item R=2: Durch Taylorentwicklung und Koeffizientenvergleich $(f=f(t,u(t)), f_t = f_t(t,u) \text{ usw.})$ folgt: \\
$c_1f + c_2f(t+ha_2, u+hb_{21}f) = (c_1+c_2) f + c_2 a_2 h f_t + c_2b_{21}hff_x + O(h^2) \\
= f + \frac{1}{2} h (f_t + f_xf) + O(h^2) \\
\Rightarrow c_1 + c_2 = 1 und c_2a_2 = \frac{1}{2} = c_2b_{21}$. Mögliche Lösungen sind z.B.:
\begin{itemize}
\item $c_1 = c_2 = \frac{1}{2}$ und $a_2 = b_{21} = 1$ (Heun'sches Verfahren zweiter Ordnung) \\
$y_n = y_{n-1} + \frac{1}{2} h_n ( f(t_{n-1}, y_{n-1}) + f(t_n, y_{n-1 + h_n f(t_{n-1}, y_{n-1})}) )$
\item $c_1 = 0, c_2 = 1, a_2 = b_{21} = \frac{1}{2}$ (modifiziertes Euler Verfahren) \\
$y_n = y_{n-1} + h_n f (t_{n - \frac{1}{2}}, y_{n-1} + \frac{1}{2} h_n f(t_{n-1}, y_{n-1}))$
\end{itemize}

\item R=3: Wir haben 8 freie Parameter und 6 Gleichungen. \\
$c_1+c_2+c_3 = 1, c_2a_2 + c_3a_3 = \frac{1}{2}, c_2a_2^2 + c_3a_3^3 = \frac{1}{3}, c_3a_2b_{32} = \frac{1}{6}, b_{21} - a_2 = 0, b_{31} - a_3 + b_{32} = 0$ \\
Mögliche Lösungen sind z.B.:
\begin{itemize}
\item das Heun'sche Verfahren dritter Ordnung: $c_1 = \frac{1}{4}, c_2 = 0, c_3 = \frac{3}{4}, a_2 = \frac{1}{3}, a_3 = \frac{2}{3}, b_{21} = \frac{1}{3}, b_{31} = 0, b_{32} = \frac{2}{3}$ \\
$y_n = y_{n-1} + \frac{1}{4} h_n (b_1 + 3 k_3)$, wobei $k_1 = f(t_{n-1}, y_{n-1}), k_2 = f(t_{n-\frac{2}{3}}, y_{n-1} + \frac{1}{3} h_n k_1), k_3 = f(t_{n-\frac{1}{3}}, y_{n-1} + \frac{2}{3} h_n k_2)$ ist.
\item das Kutta'sche Verfahren dritter Ordnung: $c_1 = \frac{1}{6}, c_2=\frac{2}{3}, c_3 = \frac{1}{6}, a_2 = \frac{1}{2}, a_3 = 1, b_{21} = \frac{1}{2}, b_{31} = -1, b_{32 = 2}$ \\
$y_n = y_{n-1} + \frac{1}{6} (k_1 + 4 k_2 + k_3)$, wobei $k_1 = f(t_{n-1}, y_{n-1}), k_2 = f(t_{n - \frac{1}{2}}, y_{n-1} + \frac{1}{2} h_n b_1), k_3 = f(t_n, y_{n-1}-h_nb_1 + 2k_nb_2)$ ist.
\end{itemize}

\item R=4: 13 freie Parameter, 11 Gleichungen. \underline{Eine} der Lösungen von mehreren ist das "klassische Runge-Kutta-Verfahren vierter Ordnung": \\
$y_n = y_{n-1} + \frac{1}{6}h_n (k_1+2k_2+2k_3+k_4)$, wobei \\
$k_1 = f(t_{n-1}, y_{n-1}), k_2 = f(t_{n-\frac{1}{2}}, y_{n-1} + \frac{1}{2} h_n k_1), k_3 = f(t_{n-\frac{1}{2}}, y_{n-1} + \frac{1}{2} h_n k_2), k_4 = f(t_n, y_{n-1} + h_n k_3)$ sind.
\end{itemize}
15.12.15: \\
Donnerstag, 18.12.25 über Zoom. Wird per E-Mail + Learnweb angesagt.\\
\\
\textbf{Lokale Konvergenz und Fehlerabschätzung:}
\subsection{Definition: Lipschitz-Stetigkeit}
Eine Einschrittformel heißt Lipschitz-stetig(L-stetig), falls ihre Verfahrensfunktion einer (gleichmäßigen) Lipschitzbedingung $(\star) ||F(h,t,x,y) - F(h,t,\tilde{x}, \tilde{y})|| \leq L (||x-\tilde{x}|| + ||y - \tilde{y}||)$ für beliebige Argumente $(t,x), (t,\tilde{x}), (t,y), (t,\tilde{y}) \in I \times \mathbb{R}^d$ genügt.

\subsection{Theorem: Diskrete Stabilität}
Eine Lipschitz-stetige Differenzenformel $(L_hy^h) = h_n^{-1} (y_n-y_{n-1}) - F(h_n, t_{n-1}, y_n, y_{n-1}) = 0 (\star\star)$ ist (diskret) stabil, d.h. für beliebige Gitterfunktionen $y^h = \{ y_n \}_{n \geq 0}, z^h = \{ z_n \}_{n \geq 0}$ gilt für genügend kleine Schrittweite $h < \frac{1}{2}L^{-1}$ die Abschätzung \\
$||y_n - z_n|| \leq e^{\kappa L (t_n - t_0)} [||y_0- z_0|| + \sum_{\nu = 1}^n h_\nu || (L_hy^h - L_hz^h)_\nu ||]$ mit Lipschitzkonstante L der Verfahrensfunktion $F(h,t,x,y)$ und der Konstanten $\kappa=4$ für allgemeine implizite Methoden. Für explizite Methoden ist $\kappa = 1$ und die Schrittweitenbedingung kann entfallen.

\subsection{Theorem: Konvergenzsatz}
Sei die Differenzenformel $(\star \star)$ in 13.12 L-stetig und konsistent mit dem Anfangswertproblem. \\
Falls $||y_0-u_0|| \to 0$ konvergiert, dann gilt $\underset{t_n \in I}{\max} ||y_n - u(t_n)|| \to 0$ für $h \to 0$ und für genügend kleines $h < \frac{1}{2}L^{-1}$ gilt die Fehlerabschätzung $||y_n-u(t_n)|| \leq e^{4L(t_n-t_0)} (||y_0-u_0|| + \sum_{\nu=1}^n h_\nu ||\tau_\nu||), 0 \leq n \leq N$. Für eine explizite Methode kann die Schrittweitenbedingung entfallen. \\
\textbf{Beweis:} \\
Da $L_hy^h = 0$ und $L_hu^h = \tau^h$ folgt die Behauptung unmittelbar aus dem diskreten Stabilitätssatz. \\
\\
\\
\textbf{Beweis 13.12:} \\
Habe ich keine Lust drauf: \\ \\
\includegraphics[scale=0.115]{lib/Beweis_13.12_1.png} \\
\includegraphics[scale=0.12]{lib/Beweis_13.12_2.png}
\\
\underline{Bemerkung:} Der Konvergenzsatz besagt, dass für L-stetige Einschrittverfahren die globale Konvergenzordnung (mindestens) gleich der lokalen Konsistenzordnung ist. Dies gilt damit z.B. für die Taylor-Verfahren, die Runge-Kutta-Verfahren. Diese genügen für L-stetiges $f(t,x)$ automatisch der Bedingung $(\star)$ in 13.11. Und man \underline{kann} zeigen, dass sie unter der Bedingung $\sum_{r=1}^R c_r = 1$ auch konsistent sind (eventuell in der Übung nochmal). Dasselbe gilt für das implizite Eulerverfahren, die Trapezregel und die sogenannte Einschritt-Mittelpunktsregel. \\
\underline{Bemerkung:} Bisher haben wir eine globale Lipschitzbedingung angenommen, d.h. z.B. $f(t,x) = x^2$ und $f(t,x) = \sqrt{x}$ sind ausgeschlossen. Es genügt jedoch \underline{folgende Bedingung:} Habe das Anfangswertproblem eine eindeutige Lösung auf dem Intervall $I=[t_0, t_0+T]$ und genügen $f(t,x)$ und $F(t,x,y)$ einer Lipschitz-Bedingung auf dem Streifen $U_\rho = \{ (t,x) \ in I \times \mathbb{R}^d | \text{ } ||x-u(t)|| \leq \rho \}$ um die Lösung $u(t)$ herum. \\
\underline{Globale Konvergenz:} Die a priori Fehlerabschätzung im Konvergenzsatz 13.13 liefert eine realistische Fehlerschranke \underline{nur}, wenn für relativ kleine Intervalle $I$. Der Term $exp(LT)$ wächst schnell für $T \to \infty$. Zudem kann man die Lipschitz-Konstante $L$ meist nur sehr grob schätzen. \\

\textbf{Globale Konvergenz} die a priori Fehlerabschätzung in 13.13 liefert eine realistische Fehlerschranke nur für relativ kleine Intervalle $I=[t_0, t_0+T]$. Der Term $exp(LT)$ wächst sehr schnell für $T \to \infty$. Zudem kann man die Lipschitzkonstante $L$ meist nur sehr grob schätzen. Wir betrachten hier wieder den Modellfall der Eulerschen Polygonzugmethode \\
$(\star) y_n = y_{n-1} + h_n f(t_{n-1}, y_{n-1})$ und wenden sie auf ein Anfangswertproblem an, für das gilt: (hier ist $( , )$ das Skalarprodukt)\\
 $||f(t,x)-f(t,\tilde{x})|| \leq l(t) ||x - \tilde{x}|| (1)$ und $-(f(t,x) - f(t,\tilde{x}), x - \tilde{x}) \geq \lambda(t) ||x-\tilde{x}|| (2)$ $\forall (t,x),(t,\tilde{x}) \in Ix\mathbb{R}^d$ und stetigen Funktionen $l(t), \lambda(t) \geq 0$. \\
Definiere $L:=\underset{I}{\max} l(t), \lambda := \underset{I}{\min} \lambda(t)$. Falls $f(t,0) = 0$, multipliziere $(\star)$ mit $y_n$ und nutze $2 ||y_n||^2 - 2 (y_{n-1}, y_n) = ||y_n||^2 + ||y_n - y_{n-1}||^2 - ||y_{n-1}||^2$, um folgende Identität zu erhalten mit $L_n := L(t_n)$ und $\lambda_n := \lambda(t_n)$: \\



$||y_n||^2 + ||y_n - y_{n-1}||^2 \leq ||y_{n-1}||^2 - 2\lambda_{n-1} h_n ||y_{n-1}||^2 + 2 h_n L_{n-1} ||y_{n-1}||$ $||y_n - y_{n-1}||\leq ||y_{n-1}||^2 - 2 \lambda_{n-1} h_n ||y_{n-1}||^2 + h_n^2 l_{n-1}^2 ||y_{n-1}||^2 + ||y_n - y_{n-1}||^2$ \\
$\Rightarrow ||y_n||^2 \leq (1+h_n^2 L_{n-1}^2 - 2 \lambda_{n-1}h_n) ||y_{n-1}||^2$. Damit bleibt die Approximation $y_n$ beschränkt bezüglich $n$, falls $(1+h_n^2L_{n-1}^2 - 2 \lambda_{n-1} h_n) \leq 1$ ist, d.h. wenn für die Schrittweite $h_n$ gilt, dass $h_n \leq \frac{2 \lambda{n-1}}{L_{n-1}^2}, n \geq 1$. \\
Falls diese Abschätzung gleichmäßig bezüglich $n$ und im strikten Sinne erfüllt ist, \underline{kann man zeigen} (für $y_0 = u_0$), dass $(\cdot) ||y_n - u(t_n)|| \leq c \underset{1 \leq \nu \leq n}{\max} \{ h_\nu \underset{I_\nu}{\max} ||u''|| \}$ ist. Der Beweis für die Polygonzugmethode ist relativ kompliziert. Daher betrachten wir den Beweis für:
\subsection{Theorem: Globale Konvergenz des impliziten Eulerverfahrens (als Bsp. zu oben Stehendem)}
Sei das Anfangswertproblem Lipschitzstetig und monoton im Sinne von $(1)$ und $(2)$. Dann sind die Lösungen des impliziten Eulerverfahrens $y_n = y_{n-1} + h_n f(t_n, y_n), n \geq 1, y_0 = u_0$ für beliebige Schrittweiten $h_n$ wohldefiniert und es gilt die Fehlerabschätzung $||y_n - u(t_n)|| \leq \frac{1}{2} \min \{ t_n - t_0, \lambda^{-1} \} \underset{1 \leq \nu \leq n}{\max} \{ h_\nu \underset{I_\nu}{\max} ||u''|| \}$ für $t_n \geq t_0$. \\
\underline{Beweis:} \\
1) In jedem Schritt des impliziten Euler-Verfahrens müssen wir ein Gleichungssystem der Form $(\star)$ $y_n - h_n f(t_n, y_n) = y_{n-1}$ lösen. Wegen $(1)$ und $(2)$ ist die Abb. $g(x) := x - h_n f(t_n,x)$ L-stetig und strikt monoton im Sinne von $(g(x - g(z), x-z) \geq \gamma ||x-z||^2$ für $x,z \in \mathbb{R}^d$ und fester Konstante $\gamma > 0$. \\
$\Rightarrow$ $(\star)$ hat eine eindeutige Lösung. \\
2) Für den Fehler $e_n = y_n - u_n$ gilt die Differenzengleichung $e_n = e_{n-1} + h_n [f(t_n, y_n) - f(t_n, u_n)] - h_n \tau_n$ mit dem Abschneidefehler $\tau_n$n des impliziten Euler-Verfahrens: $||\tau_n|| \leq \frac{1}{2} h_n \underset{I_n}{\max} ||u''||$. \\
Multipliziere mit $||e_n||^{-1} e_n$. Dann erhalten wir $||e_n|| \leq ||e_n||^1 (e_{n-1}, e_n) - \lambda_n h_n ||e_n|| + h_n ||e_n||^{-1} (\tau_n, e_n)$ \\
$\Rightarrow (1 + \lambda_n h_n) ||e_n|| \leq ||e_{n-1}|| + h_n ||\tau_n||$ bzw. $||e_n|| \leq \frac{1}{1+\lambda_n h_n} ||e_{n-1}|| + \frac{h_n}{1+\lambda_n h_n} ||\tau_n||$. \\
3) Falls $\lambda \geq 0$ (schwache Monotonie) summieren wir diese letzte Ungleichung über $\nu = 1,...,n$. Da $e_0 = 0$, erhalten wir die T-abhängige Abschätzung $||e_n|| \leq \sum_{\nu=1}^n h_\nu ||\tau_\nu|| \leq (\sum_{\nu = 1}^n h_\nu) \underset{1 \leq \nu \leq n}{\max} ||\tau_\nu|| \leq \frac{1}{2} (t_n - t_0) \underset{1 \leq \nu \leq n}{\max} \{ h_\nu \underset{I_\nu}{\max} ||u''|| \}$. \\
4) Falls $\lambda > 0$ (strikte Monotonie) folgt mit Induktion, dass $||e_n|| \leq \lambda^{-1} \underset{1 \leq \nu \leq n}{\max} ||\tau_\nu||$ bzw. $||e_n|| \leq \frac{1}{2} \lambda^{-1} \underset{1 \leq \nu \leq n}{\max} \{ h_\nu \underset{I_n}{\max} ||u''|| \}$, \underline{denn:} <Induktion> \\
$n=1: ||e_1|| \leq \frac{h_1}{1+\lambda h_1} ||\tau_1|| \leq \frac{1}{\lambda} ||\tau_1||$. Gelte die Bedingung für $n-1$. Dann folgt $||e_n|| \leq \frac{1}{\lambda h_n} ||e_{n-1}|| + \frac{h_n}{1+\lambda h_n} ||\tau_n|| \leq \frac{1}{1+\lambda h_n} \lambda^{-1} \underset{1 \leq \nu \leq n-1}{\max} ||\tau_\nu|| + \frac{h_n}{1+\lambda h_n} ||\tau_n|| \leq \frac{1}{\lambda} \underset{1 \leq \nu \leq n}{\max} ||\tau_\nu||$. \\

Gute, vernünftige Verfahren sollten jede (exponentiell) stabile Lösung global approximieren, egal ob das Problem selbst monoton ist oder nicht. \underline{Frage:} Lassen sich globale Fehlerabschätzungen der Art $(\cdot)$ herleiten \underline{allein} mit Hilfe der angenommenen (exponentiellen) Stabilität der Lösung $u(t)$ und ohne weitere Vorausssetzungen? Antwort gibt das folgende Theorem:

\subsection{Theorem: Globale Konvergenz}
Das L-stetige AWP habe eine (globale) exponentiell stabile Lösung mit Stabilitätsparametern $\delta, A, \alpha$.

[Eine globale Lösung $u$ eines AWPs heißt exponentiell stabil, wenn es Konstanten $\delta, \alpha, A$ gibt, sodass zu jedem Zeitpunkt $t_\star \geq t_0$ und jedem $w_\star \in \mathbb{R}^d$ mit $||w_\star|| < \delta$ jede (globale) Lösung $v$ des gestörten AWPs $v'(t)=f(t,v(t)), t \geq t_\star, v(T_\star)=u(t_\star) + w_\star$ \underline{global ist} und $||(v-u)(t)|| \leq A e^{-\alpha(t-t_\star)} ||w_\star||$. \underline{Bemerkungen:} 1) Asymptotische Stabilität ist eine schwächere Stabilitätsdefinition. 2) \underline{Alle} Lösungen eines (lokal) L-stetigen und (stark) monotonen AWPs sind global und exponentiell stabil mit $\delta$ beliebig und $\alpha = A = 1$. Falls $\underset{t \geq t_0}{\sup} ||f(t,0)|| < \infty \Rightarrow$ alle Lösungen sind gleichmäßig beschränkt.]

Für jedes Einschrittverfahren, welches mit dem AWP konsistent ist und einer Lipschitzbedingung genügt, gibt es dann positive Konstanten $h_0$ und $K$ (unabhängigg von $T$), sodass für $h := \underset{I}{\sup} h_n \leq h_0$ gilt: $\underset{t_n \in I}{\max} ||y_n - u(t_n)|| \leq K \underset{t_n \in I}{\max} ||\overline{\tau_n}||$, wobei $\overline{\tau_n}$ das Maximum des Abschneidefehlers für alle möglichen Lösungen der \DGL bezeichnet, die in der Umgebung $U_{\delta A} = \{ (t,x) \in Ix\mathbb{R}^d |$ $||x-u(t)|| < \delta A \}$ des Graphen von u verlaufen. (Beweis evtl. später, eher nicht)

\textbf{Schrittweitenkontrolle:} Wie kann man während einer Simulation die Schrittweite $h_n$ so wählen, dass zu einer vorgegebenen Fehlertoleranz $TOL>0$ auf dem Intervall $I$ die Schranke $\underset{t_n \in I}{\max} ||e_n|| \leq TOL$ erreicht wird? Hier soll $TOL$ deutlich größer als die Maschinengenauigkeit $eps$ gewählt sein, z.B. $TOL > \underset{t_n}{\max} \{ h_n^{-1} ||y_{n-1}eps \}$.

Wir starten mit der a priori Fehlerabschätzung $\underset{t_n \in I}{\max} ||e_n|| \leq K \sum_{t_n \in I} h_n ||\tau_n|| \leq KT \underset{t_n \in I}{\max} ||\tau_n||$. Wir \underline{nehmen an}, dass Schätzungen für die lokalen Abschneidefehler $\tau_n$ bekannt sind bzw. für die "Hauptabschneidefehlerfunktion" $\tau_n^{(m)} = \tau^{(n)} (t_n)$. Auf dem Intervall $[t_{n-1}, t_n]$ zur Schrittweite $h_n$ ist $\tau_n = \tau^{(m)}(t_n) h_n^m + O(h_n^{m+1})$.

Für das Taylor-Verfahren ist zum Beispiel $\tau^{(m)}(t_n) = \frac{1}{(m+1)!} u^{(m+1)}(t_{n-1})$. Obige Schätzungen \underline{kann} man berechnen. (Ab hier nicht in der Klausur glaube ich)
\begin{enumerate}
\item Strategie: Wähle die Schrittweite $h_n$ wie folgt: $Kh_n^m ||\tau_n^{(m)}|| \approx \frac{TOL}{T}$ bzw. $h_n \approx (\frac{TOL}{KT||\tau_n^{(m)}||})^{1/m}$. Dann folgt wie gewünscht, dass $\underset{t_n \in I}{\max} ||e_n|| \approx K \sum_{t_n \in I} h_n \{ h_n^m || \tau_n^{(m)}|| \} \approx \frac{TOL}{T} \sum_{t_n \in I} h_n$. Die Anzahl der durchzuführenden Zeitschritte ergibt sich aus:

$N=\sum_{t_n \in I} h_n h_n^{^1} \approx \sum_{t_n \in I} h_n (\frac{KT ||\tau_n^{(m)}||}{TOL})^{1/m} = (\frac{KT}{TOL})^{1/m} \sum_{t_n \in I} h_n ||\tau_n^{(m)}||^{1/m}$. \\Da $\tau_n^{(m)} \approx u^{(m+1)}(t_{n-1})$ folgt, dass $N \approx (\frac{KT}{TOL})^{1/m} \int_I ||u^{(m+1)}||^{1/m} dt$.
\item Strategie: Wähle $h_n$ so, dass $K h_n^{m+1} ||\tau_n^{(m)}|| \approx \frac{TOL}{N}$ bzw. $h_n \approx (\frac{TOL}{KN||\tau_n^{(m)}||})^{\frac{1}{m+1}}$. Hier ist N (Anzahl der Zeitschritte) noch unbekannt \\$\Rightarrow \underset{t_n \in I}{\max} ||e_n|| \approx K \sum_{t_n \in I} \{ h_n^{m+1} ||\tau_n^{(m)}|| \} \approx \frac{TOL}{N} \sum_{t_n \in I} 1$ und \\$N \approx \sum_{t_n \in I} h_n (\frac{KN||\tau_n^{(m)}||}{TOL})^{\frac{1}{(m+1)}} = (\frac{KN}{TOL})^{\frac{1}{m+1}} \sum_{t_n \in I} h_n ||\tau_n^{(m+1)}||^{\frac{1}{(m+1)}}$. \\Da $\tau_n^{(m)} \approx u^{(m+1)}(t_{n-1}) \Rightarrow N^{\frac{m}{m+1}} = N^{1-\frac{1}{m+1}} \approx (\frac{K}{TOL})^{\frac{1}{m+1}} \int_I ||u^{(m+1)}||^{\frac{1}{m+1}} dt \\ \Rightarrow N \approx (\frac{K}{TOL})^{\frac{1}{m}} (\int_I ||u^{(m+1)}||^{\frac{1}{m+1}} dt)^{\frac{m+1}{m}}$. Da N a priori nicht bekannt ist, muss N geschätzt werden und im Verlauf mehrerer Simulationen angepasst werden. Das erscheint aufwändiger als Strategie 1.
\end{enumerate}

Beide Strategien zur Schrittweitenwahl sind asymptotisch gleich effizient. Die globale Fehlertoleranz $TOL$ wird mit $N \approx TOL^{-\frac{1}{m}}$ Zeitschritten erreicht, allerdings mit kleinen Unterschieden bei den Konstanten. \underline{Was bedeutet dies?}

\underline{Beispiel:} $m=1$, Eulersche Polygonzugmethode. \\
1. Strategie: $N \approx \frac{KT}{TOL} \int_I ||u''|| dt$, \\
2. Strategie: $N \approx \frac{K}{TOL}(\int_I ||u''||^{\frac{1}{2}}dt)^2$. Wie geht die Regularität der exakten Lösung in die Schrittzahl ein? Die 2. Strategie ist dann ökonomischer im Vergleich zur 1. Strategie, wenn $(\int_I ||u''||^{\frac{1}{2}} dt)^2 << T \int_I ||u''|| dt$ ist.

\underline{Beispiel:} singuläre Lösungen, deren zweite Ableitung nicht integrabel ist, z.B. $u(t) = (1-t)^{\frac{1}{2}}$.

\underline{\textbf{Numerische Stabilität}} Ein Lipschitz-stetiges und (strikt) monotones Anfangswertproblem $u'(t) = f(t,u(t)), t \geq t_0, u(t_0) = u_0$ hat im Fall, dass $\underset{t > 0}{\sup} ||f(t,0)|| < 0$ ist, eine globale, gleichmäßig beschränkte Lösung. Falls $f(t,0) \equiv 0$ ist, dann fällt die Lösung exponentiell gegen $0$ ab. Sei nun $L(t)$ die Lipschitz-Konstante und $\lambda(t)$ die Monotonie-Konstante von $f(t,\cdot)$. Man kann zeigen, dass das Polygonzugverfahren eine analoge Eigenschaft besitzt,  \underline{wenn die strikte Schrittweitenbedingung} $\underset{n \geq 0}{\inf} \{ \frac{2 \lambda_{n-1}}{h_n L_{n-1}^2} \} > 0$ erfüllt ist. Das heißt für solche Schrittweiten ist das Verfahren "numerisch stabil".

\textbf{Skalare Testgleichung:} $u'(t) = \lambda u(t) = f(t,u(t)), \lambda \in \mathbb{C}, L = |\lambda|$. Hier steht $\lambda \in \mathbb{C}$ für die i.A. komplexen Eigenwerte der Jacobi-Matrix der rechten Seite. Wie zuvor: Eulersche Polygonzugmethode.

Für $\lambda \in \mathbb{R}, \lambda < 0$ gilt: $y_n = (1+h_n \lambda) y_{n-1} = ... = (1+h_n \lambda)^n y_0$. Das heißt für $h > 2 \frac{|\lambda_{n-1}|}{L_{n-1}^2} = \frac{2}{|\lambda|}$ wächst diskrete Lösung exponentiell. [Erinnerung: Schrittweitenbedingung $\underset{n \geq 0}{\inf} \{ \frac{2 \lambda_{n-1}}{h_n L_{n-1}^2} >0 \}$]

Für $h=2 \frac{|\lambda_{n-1}|}{L_{n-1}^2} = \frac{2}{|\lambda|}$ bleibt sie beschränkt, im Absolutbetrag sogar konstant.

Für $h < 2 \frac{|\lambda_{n-1}|}{L_{n-1}^2} = \frac{2}{|\lambda|}$ fällt sie exponentiell.

\underline{Illustrition:} $u'(t) = -200t [u(t)]^2, t \geq 0, u(0) = 1$ hat die Lösung $u(t) = (1+100t^2)^{-1}$. Der Wert $u(3) = \frac{1}{901}$ soll mithilfe des klassischen Runge-Kutta-Verfahrens approximiert werden. Das Verfahren konvergiert. \underline{Vermutung:} Also können wir mit relativ großen Schrittweiten arbeiten. \underline{Dies ist falsch}.
...
15 & 0,2 & $\sim$ overflow($10^{38}$) \\
\hline
20 & 0,1538 & $\sim 7 \cdot 10^{-5}$ \\
19 & 0,1579 & $\sim$ overflow($10^{38}$)
\end{tabular}

Es existiert eine \underline{numerische} Instablität, wenn die Schrittweite zu groß ist. Wie können wir dies besser verstehen? \\
\\
\underline{Lineare Stabilitätsanalyse}: \underline{Intuition}: Ein Differenzenverfahren ist "numerisch stabil" für feste $h$, wenn im Falle $\underset{t>0}{\sup} ||u(t)|| < \infty$ auch $\underset{n>0}{\sup} ||y_n|| < \infty$ ist. 

\underline{Betrachte unser Testproblem:} $u'(t) = \lambda u(t)$, also $u(t) = u_0 e^{\lambda t}$. Das Verhalten der Lösung für $t \to \infty$ ist durch 
\begin{itemize}
\item $Re \lambda < 0 \Rightarrow |u(t)| = |u_0| e^{Re \lambda t} \to 0$
\item $Re \lambda = 0 \Rightarrow |u(t)| = |u_0| e^{Re \lambda t} \equiv |u_0|$
\item $Re \lambda > 0 \Rightarrow |u(t)| = |u_0| e^{Re \lambda t} \to \infty$
\end{itemize}
charakterisiert.

\subsection{Definition: Absolute Stabilität}
Eine Einschrittmethode heißt "absolut stabil" für ein $\lambda f \neq 0$, wenn ihre Anwendung auf das skalare Testproblem $u'(t) = \lambda u(t), \lambda \in \mathbb{C}$ für $Re \lambda \leq 0$ beschränkte Näherungen erzeugt, d.h. $\underset{n \geq 0}{\sup} |y_n| < \infty$. Für die Polygonzugmethode liegt also absolute Stabilität genau dann vor, wenn für den Faktor $w := w(\lambda h) := (1+\lambda h)$ gilt, dass $|w| \leq 1$. Wir nennen $SG := \{ z = \lambda h \in \mathbb{C}|$ $|w(z)| \leq 1 \}$ das Gebiet absoluter Stabilität (oder Stabilitätsgebiet) einer Einschrittformel. Für festes $\lambda$ mit $Re \lambda \leq 0$ muss die Schrittweite $h$ der Polygonzugmethode so gewählt werden, dass $\lambda h \in SG$ ist. Ansonsten wächst die Näherungslösung $y_n$ für $n \to \infty$ exponentiell, \underline{obwohl} die exakte Lösung beschränkt ist oder exponentiell abfällt. Wir untersuchen nun die numerische Stabilität der Taylorformeln anhand unseres Testproblems $u'(t) = \lambda u(t), \lambda \in \mathbb{C}$.

\underline{Taylormethode der Stufe R}: $y_n = y_{n-1} + h \sum_{r=1}^R \frac{h^{r-1}}{r!} f^{r-1}(t_{n-1}, y_{n-1}) = y_{n-1} + h \sum_{r=1}^R \frac{h^{r-1}}{r!} \lambda^r y_{n-1}$. Der Verstärkungsfaktor ist $w = \sum_{r=0}^R \frac{(\lambda h)^r}{r!}$. Es ist zumeist schwierig, das volle Stabilitätsgebiet $SG = \{ z \in \mathbb{C} |$ $|w(z)| \leq 1 \}$ zu bestimmen. Daher betrachten wir hier nur das Stabilitätsintervall $SI = \{ z \in \mathbb{R} |$ $|w(z)| \leq 1 \}$.

Es gilt: $SI = [-2, 0]$ für $R=1$, $[-2, 0]$ für $R=2$, $[-2.51..., 0]$ für $R=3$, $[-2.78..., 0]$ für $R=4$.

Sei nun $F(h;t,x,)$ die Verfahrensfunktion eines R-stufigen Runge-Kutta-Verfahren der Ordnung $m=R\leq4$. Nach der Konstruktion gilt, dass $F(h;t,u) = \sum_{r=1}^R \frac{h^{r-1}}{r!} f^{(r-1)} (t,u) + O(h^R)$. Für unser Testproblem ist $F(h;t,u) = \sum_{r=1}^R c_r k_r (h;t,u)$ ein Polynom in h der Ordnung $R-1$ $\Rightarrow F(h;t,u) = \sum_{r=1}^R \frac{h^{r-1}}{r!} f^{(r-1)}(t,u) + 0$. Damit ist der Verstärkungsfaktor $w$ der Runge-Kutta Formeln der Ordnung $m=R\leq4$ derselbe wie der der entsprechenden Taylorformeln. Aus unserer Stabilitätsanalyse folgt, dass für $Re \lambda << -1$ die Stabilität der durch die Runge-Kutta-Verfahren erzeugten Lösungen eine entsprechend kleine Schrittweite $h$ erfordert. Dann wäre eine Formel, mit einem in der komplexen Ebene möglichst weit nach links reichenden Stabilitätsgebiet optimal.

\subsection{Definition: A-stabil}
Eine Differenzenmethode heißt A-stabil, wenn für das Stabilitätsgebiet $\{ z \in \mathbb{C} | Re z \leq 0 \} \subset SG$ gilt. Man \underline{kann} zeigen, dass explizite Methoden \underline{nicht} A-stabil sein können. Man \underline{kann} zeigen, dass die implizite Eulermethode und die Trapezregel A-stabil sind.

$y_n = y_{n-1} + h f(t_n, y_n); y_n = y_{n-1} + \frac{1}{2} h (f(t_n, y_n) + f(t_n, y_{n-1}))$

\underline{Nutzung der linearen Stabilitätsanalyse}: Betrachte das AWP $u'(t) = f(t,u(t)), t \geq t_0, u(t_0) = u_0$ (nicht notwendig monoton). Dieses sei mit einem Einschrittverfahren diskretisiert

$y_n = y_{n-1} + h_n F(h_n; t_n, y_n, y_{n-1}, n \geq 0, y_0 = u_0$. Hier sei die Verfahrensfunktion L-stetig. Eine (globale) Lösung $(y_n)_{n \geq 0}$ heißt (numerisch) stabil, wenn für jede Lösung $(z_n)_{n \geq n^\star}$ von $z_n = z_{n-1} + h_n F(h_n; t_n, z_n, z_{n-1}), n \geq n^\star$, $z_{n^\star} = y_{n^\star} + w_\star$ zu einem Zeitpunkt $t_{n^\star} \geq t_0$ mit hinreichend kleiner Störung $||w_\star|| \leq \delta$ gilt, dass $||z_n - y_n|| \to 0$ für $n \to \infty$.

\underline{Bemerkung} Analog zum kontinuierlichen Fall nennen wir die Lösung $(y_n)_{n \geq 0}$ einer Differenzenapproximation \underline{exponentiell stabil}, wenn für jede Lösung der gestörten Differenzengleichung $||z_n - y_n|| \leq A e^{- \alpha (t_n - t_\star)} ||w_\star||, n \geq n_\star$ gilt.

\underline{Hypothese}: Die (globale) Lösung $u$ des allgemeinen AWP ist asymptotisch stabil.

..............

\subsection{Discontinued}
Klausurinfos am 22.01.26 werden noch ergänzt.

\newpage
\section{Nützlicher Stuff, z.B. Infos zur Klausur}
Random Stuff:
\begin{itemize}
\item negativer Realteil impliziert stabil
\item Lyapunov Funktionen werden in der Klausur gegeben
\item Kein solcher/so langer Beweis wie zu 12.7 in der Klausur
\item Aus Feld 13 eine Aufgabe in der Klausur
\item Was besonders zu lernen ist, wird in einer VL bekannt gegeben
\item Maximal eine Numerik-Aufgabe in der Klausur
\item Am 19.01.2026 mehr Infos zur Klausur
\item Bei Unter- und Oberfunktionen dürfen wir immer mit $\leq$, $\geq$ rechnen
\end{itemize}

In der Musterlösung zu den Weihnachtsaufgaben:

Vergleichsprinzip durch Over- und Unterlösungen

in der Musterlösung $\leq$, $\geq$

in der VL: (strikte, damals nicht strikt genannt) Ober- und Unterlösungen: $<$, $>$

Was wir uns für die Klausur nochmal anschauen sollten(kommt nicht garantiert genau so dran, nur sehr wahrscheinlich: "Bitte noch einmal gut üben")(evtl. Donnerstag 29.01. noch mehr Infos):

\begin{itemize}
\item Phasenraumanalyse (leichter als auf dem Beispielblatt für Weihnachten)
\item Linearisieren, Stabilität etc.
\item Existenz und Eindeutigkeit für konkrete Probleme (global, lokal) (evtl. Fixpunktsatz?? $\to$ Banachraum)
\item Lipschitz $\neq$ Differenzierbar; diffbar ist stärker; (global, lokal)
\item Ober- und Unterlösungen
\item Poincare Bendixson / Bendixson Kriterium
\item Konsistenz/Konsistenzordnung (wenn Numerik, dann das hier)
\end{itemize}

180 Minuten wird geschrieben, 120 Minuten werden planmäßig benötigt
 \\

Nochmalige Betonung(29.01.2026):
\begin{itemize}
\item 2-dimensionale autonome lineare DGL-Systeme
\begin{itemize}
\item Gleichgewichtsbestimmmung
\item Typen der Gleichgewichte
\item und deren Stabilität(stabil, instabil, asymptotisch stabil)
\end{itemize}

\item Phasenraumanalysen - in allen Details, mit klaren mathematischen Begründungen (nichtlineare Systeme)
\begin{itemize}
\item wenn in der Klausur, dann wird genannt, in welchen Quadranten gearbeitet werden soll
\end{itemize}

\item Existenz und Eindeutigkeit, oder nicht (lokal) $\leadsto$ Lipschitz, diffbar, Picard-Lindelöf, Blow-Up, Ober-/Unterlösungen (könnte lang werden)

\item Nichtlineare Systeme (Lyapunov-Funktionen(nicht raten) $\leadsto$ Stabilität der Gleichgewichte); Bendixson-Kriterium (!kein Poincaré-Bendixson!)

\item "Numerische Analysis"
\begin{itemize}
\item Verfahren $y_n = y_{n-1} + h_n F(h_n; t_{n-1}, y_n, y_{n-1}$
\item lokaler Diskretisierungsfehler/Abschneidefehler $\tau_n$ (darum geht es auch maximal) $\leadsto$ $\tau_n = \frac{u(t_n) - u(t_{n-1})}{h_n} - F(h_n; t_{n-1}, u(t_n), u(t_{n-1}))$
\item Konsistenz: $\underset{t_n \in I}{\max} ||\tau_n|| \to 0$ für $h \to 0$
\item Konsistenzordnung: $\underset{t_n \in I}{\max} ||\tau_n|| = O(h^m) \leq C \cdot h^m$ für $h \to 0$ (m ist Konsistenzordnung)
\end{itemize}

\item 4 Aufgaben, aber Themen können übergreifend sein

\item eher sinnlos Definitionen abzufragen
\end{itemize}
